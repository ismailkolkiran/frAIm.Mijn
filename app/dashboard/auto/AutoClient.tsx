...
export default function AutoClient({ vehicle, maintenance }: { vehicle: Vehicle; maintenance: Log[] }) {
  ...
  async function submit(event: FormEvent) {
    ...
    const r = await fetch("/api/auto/maintenance", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ onderhoudsdatum, type, beschrijving, kosten: kosten ? Number(kosten) : null, garagenaam: garagenaam || null, notities: null }) });
    const p = await r.json();
    ...
    setOnderhoudsdatum(""); setType("onderhoud"); setBeschrijving(""); setKosten(""); setGaragenaam("");
    await refresh();
  }
