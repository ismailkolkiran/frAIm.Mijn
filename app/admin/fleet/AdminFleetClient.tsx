...
export default function AdminFleetClient({initialVehicles,initialMaintenance,users}:{initialVehicles:Vehicle[];initialMaintenance:Maintenance[];users:User[]}){
  ...
  async function addVehicle(event:FormEvent){event.preventDefault();const r=await fetch('/api/admin/fleet',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type,merk:merk||null,model:model||null,nummerplaat:nummerplaat||null,status:'beschikbaar'})});if(!r.ok){window.alert('Voertuig toevoegen mislukt.');return;}setMerk('');setModel('');setNummerplaat('');await refresh();}
  async function assign(vehicleId:number,userId:number|null){const r=await fetch('/api/admin/fleet/assign',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({vehicleId,userId})});if(!r.ok){const p=await r.json().catch(()=>({error:'Toewijzing mislukt.'}));window.alert(p.error??'Toewijzing mislukt.');return;}await refresh();}
