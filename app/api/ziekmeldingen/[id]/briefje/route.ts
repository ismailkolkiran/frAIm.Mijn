...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const secureUrl = await uploadToCloudinary(file, "myfraim/sick-notes", "auto");
    await attachDoctorNote(Number(params.id), user.id, secureUrl);

    return NextResponse.json({ ok: true, secureUrl });
  } catch (error) {
    console.error("Doctor note upload error:", error);
    return NextResponse.json({ error: "Upload mislukt. Controleer CLOUDINARY_URL configuratie." }, { status: 500 });
  }
}
